...
To copy a file:
python copy_to_azure.py -in_fn [string of full path] > az.log &
...
parser.add_argument('-in_fn', type=str) # full path to input file
...
Ldir = Lfun.Lstart()
if in_fn == None:
    in_fn = Ldir['data'] / 'accounts' / 'az_testfile.txt'
else:
    in_fn = Path(in_fn)
if out_name == None:
    out_name = in_fn.name
