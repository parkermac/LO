...
in_dict['node_num'] = int(np.min(1, int(in_dict['np_num'] / in_dict['cores_per_node'])))

## create lo_batch.sh - batch job script  ##########################

f  = open(pth / 'klone_batch_BLANK0.sh','r')
f2 = open(Path(in_dict['roms_out_dir']) / 'klone_batch0.sh','w')
