...
This is for the surface extraction seen on the IOOS EDS Viewer.

Testing on mac:
run post_main.py -gtx cas6_v3_lo8b -ro 2 -d 2019.07.04 -r backfill -job surface1 -test True

Run for real on apogee:
python post_main.py -gtx cas6_v0_u0mb -ro 0 -d [today's date string] -r forecast -job surface1
...
out_fn0 = out_dir0 / (box_job + '_surf_' + ds0 + '_' + ds1 + '.nc')
