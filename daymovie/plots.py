...
def P1(Q, M):
    ...
    if vn == 'speed':
        ...
        px, py, fld = dm_pfun.get_arag(ds, Q, aa, nlev) # why get px and py?
    else:
    ...
    if Q['tracks']:
        ...
        ax.plot(tr_ds['lon'][:iot+1,:].values, tr_ds['lat'][:iot+1,:].values,'-r', alpha=.5, lw=1)
        ax.plot(tr_ds['lon'][iot,:].values, tr_ds['lat'][iot,:].values,'o', mec='k', mfc='r')
        tr_ds.close()
...
def Phab(Q, M):
    ...
    dm_pfun.add_coast(ax1, lw=1.5)
    dm_pfun.add_bathy_contours(ax1, ds, txt=False, depth_levs=[200])
    if Q['tracks']:
        tr_ds = nc.Dataset(Q['tr_fn'])
        iot = zfun.find_nearest_ind(tr_ds['ot'][:], T['ocean_time'])
        if iot > 2:
