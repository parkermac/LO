...
This version is based on driver_roms3.py, but takes a new approach to doing
multiple days. The goal is to have this run on one of the compute nodes
instead of the head node. More importantly it runs by spawning a new job
(of itself) when a day is done in order to do the next day. This avoids
having a single driver that is expected to run for many days. Hopefully
it will be more reliable for long hindcasts.
...
if args.run_type == 'forecast':
    ...
    done_fn = Ldir['LO'] / 'driver' / ('forecast3_done_' + ds0 + '.txt')
    if done_fn.is_file():
...
# Launch the next day if needed
dt0 = dt + timedelta(days=1)
if dt0 <= dt1:
    cmd_list = ['python3', str(Ldir['LO'] / 'driver' / 'driver_roms_long.py'),
                '-g', args.gridname, '-t', args.tag, '-x', args.ex_name,
                '-r', 'backfill', '-s', 'perfect',
                '-0', dt0.strftime(Lfun.ds_fmt),
                '-1', dt1.strftime(Lfun.ds_fmt),
                '-bu', str(blow_ups), '-np', str(args.np_num),
                '-short_roms', str(args.short_roms)]
    proc = subprocess.Popen(cmd_list, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # stdout, stderr = proc.communicate()
    # messages(stdout, stderr, 'Start the next day', args.verbose)
    # sys.stdout.flush()
    # sys.exit() # not needed?
