"""
Code to run more post-processing jobs for a forecast day.

>> It is motivated by the wgh nested model forecasts, which appear later in the day. <<
...
Test on apogee:
python driver_post1.py -gtx wgh2_t0_xn0b -r forecast -ro 0 -test True < /dev/null > test_post.log &

To run for real on apogee
python driver_post1.py -gtx wgh2_t0_xn0b -r forecast -ro 0 < /dev/null > post.log &
...
parser.add_argument('-d', '--date_string', default='', type=str) # e.g. 2023.09.14
...
maxcount=1440; sleeptime=60 # will keep looking for 24 hours, e.g. 9 AM to 9 AM the next day
...
if Ldir['testing'] == True:
    ...
    job_list = ['daymovie2','layers2','drifters_wgh']
