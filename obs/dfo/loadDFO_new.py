...
"""
edited to return CTD and bottle oxygen
"""

def loadDFO_CTD(basedir='/ocean/shared/SalishSeaCastData/DFO/CTD/', dbname='DFO_CTD.sqlite',
        datelims=()):
    """
...
def loadDFO_bottle(basedir='.', dbname='DFO.sqlite',
    ...
    # df1=pd.DataFrame(qry.all())
    # df1['Z']=np.where(df1['Depth'].values>=0,df1['Depth'].values,-1.0*gsw.z_from_p(p=df1['Pressure'].values,lat=df1['Lat'].values))
    df1 = pd.read_sql(qry.statement, engine)
